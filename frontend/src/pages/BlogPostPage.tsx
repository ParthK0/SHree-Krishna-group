    ...
    return (
      <div className="w-full bg-[#ECE6DD] min-h-screen py-20 px-4 text-center">
        <div className="max-w-xl mx-auto bg-white rounded-3xl p-8 border border-[#e2dacd] shadow-lg">
          ...
          <div className="space-y-2 text-left mb-6">
            {allPosts.map((p) => (
              <Link
                key={p.slug}
                to={`/blog/${p.slug}`}
                className="block p-3 rounded-xl border border-[#ece6dd] hover:border-[#062448] hover:bg-[#EBF2F9] text-xs font-bold text-[#1a1f1b] transition-all"
              >
                {p.title}
              </Link>
            ))}
          ...
          <Link
            to="/blog"
            className="px-6 py-2.5 rounded-xl bg-[#062448] text-white text-xs font-bold font-['Manrope'] uppercase tracking-wider"
          >
            Back to All Guides
          </Link>
        </div>
    ...
    <div className="w-full bg-[#ECE6DD] min-h-screen text-[#1a1f1b]">
      ...
      <div className="bg-[#E4DDD3] border-b border-[#d8d0c3] py-3 px-4 md:px-12 text-xs font-['Manrope'] text-neutral-600">
        <div className="max-w-5xl mx-auto flex items-center gap-2 flex-wrap">
          <Link to="/" className="hover:text-[#062448]">Home</Link>
          <ChevronRight size={13} />
          <Link to="/blog" className="hover:text-[#062448]">Logistics Blog</Link>
          <ChevronRight size={13} />
      ...
      <header className="bg-gradient-to-b from-[#E4DDD3] to-[#ECE6DD] pt-10 pb-12 px-4 md:px-12 border-b border-[#d8d0c3]">
        <div className="max-w-4xl mx-auto space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <span className="px-3 py-1 rounded-full bg-[#062448] text-white text-xs font-bold font-['Space_Mono'] uppercase">
              {post.category}
            </span>
          ...
          <div className="pt-4 flex flex-wrap items-center justify-between gap-4 border-t border-[#d8d0c3]">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-[#062448] text-white flex items-center justify-center font-bold text-sm font-['Space_Mono']">
                DS
              </div>
              <div>
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-12 py-12">
        ...
        <div className="bg-white rounded-3xl p-6 md:p-8 border border-[#e2dacd] shadow-md mb-12">
          <div className="flex items-center gap-2 text-xs font-bold uppercase font-['Space_Mono'] text-[#062448] mb-3">
            <Sparkles size={16} className="text-[#E9A015]" />
            <span>Executive Summary & Key Takeaways</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
            {post.keyTakeaways.map((item, idx) => (
              <div key={idx} className="flex items-start gap-2.5 text-xs sm:text-sm text-neutral-700 font-['Manrope']">
                <CheckCircle2 size={16} className="text-[#062448] shrink-0 mt-0.5" />
                <span>{item}</span>
              </div>
            ))}
          </div>
        ...
        <article className="space-y-12">
          {post.sections.map((section, sIdx) => (
            <section key={sIdx} className="bg-white rounded-3xl p-6 md:p-10 border border-[#e2dacd] shadow-sm space-y-5">
                ...
                <ul className="space-y-2.5 pt-2">
                  {section.bulletPoints.map((bp, bIdx) => (
                    <li key={bIdx} className="flex items-start gap-2.5 text-xs sm:text-sm text-neutral-700 font-['Manrope']">
                      <span className="w-1.5 h-1.5 rounded-full bg-[#062448] shrink-0 mt-2" />
                      <span>{bp}</span>
                    </li>
                  ))}
                </ul>
                ...
                <div className="pt-4">
                  <div className="overflow-x-auto rounded-2xl border border-[#e2dacd]">
                    <table className="w-full text-left font-['Manrope'] text-sm">
                      <thead>
                        <tr className="bg-[#FFF9E6] border-b border-[#E9A015]/40 text-xs font-['Space_Mono'] uppercase text-[#8a6500]">
                      ...
                      <tbody className="divide-y divide-[#ece6dd]">
                        {DELHI_NCR_7_TON_RATES.map((item) => (
                          <tr key={item.sNo} className="hover:bg-[#fbf9f6]">
                            ...
                            <td className="py-3 px-4 text-right font-extrabold text-[#062448] font-['Space_Mono'] whitespace-nowrap">
                              ₹{item.rateRange}
                            </td>
                            <td className="py-3 px-4 text-center">
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="px-3 py-1 rounded-lg bg-[#062448] text-white text-xs font-bold font-['Manrope'] hover:bg-[#0c532b]"
                              >
                                Book
                              </a>
                            </td>
                        ...
                        {PAN_INDIA_RATES.map((item) => (
                          <tr key={item.sNo} className="hover:bg-[#fbf9f6]">
                            ...
                            <td className="py-3 px-4">
                              <span className="px-2.5 py-0.5 rounded-lg bg-[#EBF2F9] text-[#062448] font-['Space_Mono'] font-bold text-xs">
                                {item.rateRange}
                              </span>
                            </td>
                            ...
                            <td className="py-3 px-4 text-right font-extrabold text-[#062448] font-['Space_Mono'] text-xs sm:text-sm">
                        ...
                        {PARCEL_COURIER_FREIGHT_RATES.map((item) => (
                          <tr key={item.sNo} className="hover:bg-[#fbf9f6]">
                            ...
                            <td className="py-3 px-4 text-xs font-['Space_Mono']">
                              <span className="px-2 py-0.5 rounded bg-[#EBF2F9] text-[#062448] font-semibold">
                                {item.badge}
                              </span>
                            </td>
                            <td className="py-3 px-4 text-xs text-neutral-600 max-w-xs">{item.idealFor}</td>
                            <td className="py-3 px-4 text-right font-extrabold text-[#062448] font-['Space_Mono'] text-sm sm:text-base whitespace-nowrap">
                              {item.rate}
                            </td>
                            <td className="py-3 px-4 text-center">
                              <a
              ...
              {/* Callout Box */}
              {section.calloutBox && (
                <div className="p-5 rounded-2xl bg-[#EBF2F9] border border-[#062448]/20 space-y-1.5 mt-4">
                  {section.calloutBox.badge && (
                    <span className="text-[10px] font-bold font-['Space_Mono'] text-[#062448] uppercase tracking-wider block">
                      {section.calloutBox.badge}
                    </span>
                  )}
        ...
        {post.faqs.length > 0 && (
          <div className="mt-12 bg-white rounded-3xl p-6 md:p-10 border border-[#e2dacd] shadow-sm">
            <div className="mb-6">
              <span className="text-xs font-bold uppercase tracking-wider text-[#062448] font-['Space_Mono'] block mb-1">
                Frequently Answered
              </span>
                ...
                return (
                  <div key={idx} className="rounded-2xl border border-[#ece6dd] overflow-hidden">
                    <button
                      ...
                      className="w-full text-left px-5 py-3.5 flex items-center justify-between font-bold text-sm text-[#1a1f1b] hover:text-[#062448] transition-colors"
                    >
                      <span>{faq.question}</span>
                      {isOpen ? <ChevronUp size={18} className="text-[#062448] shrink-0" /> : <ChevronDown size={18} className="text-neutral-400 shrink-0" />}
                    </button>
                    {isOpen && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {post.relatedRouteSlugs.map((rSlug) => (
                <Link
                  key={rSlug}
                  to={`/${rSlug}`}
                  className="p-4 rounded-2xl bg-white border border-[#e2dacd] hover:border-[#062448] hover:shadow-md transition-all flex items-center justify-between group"
                >
                  <div className="flex items-center gap-2">
                    <Truck size={16} className="text-[#062448]" />
                    <span className="font-bold text-xs uppercase font-['Space_Mono'] text-[#1a1f1b] group-hover:text-[#062448]">
                      {rSlug.replace(/-/g, ' ')}
                    </span>
                  </div>
                  <ChevronRight size={15} className="text-neutral-400 group-hover:text-[#062448]" />
                </Link>
              ))}
            </div>
          </div>
