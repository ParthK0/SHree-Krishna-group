...
const StatCardItem: React.FC<{ item: StatItem; triggered: boolean }> = ({ item, triggered }) => {
  ...
  return (
    <div className="bg-[#071F35] border border-neutral-800 hover:border-[#0B3A66] p-4 sm:p-5 rounded-xl shadow-lg transition-all duration-300 flex flex-col justify-between group">
      {/* Top Row: Icon + Micro Sparkline */}
      <div className="flex items-center justify-between mb-3">
        <div className="w-8 h-8 rounded-lg bg-neutral-900 flex items-center justify-center text-[#85B7EB] border border-neutral-800 group-hover:border-[#0B3A66] transition-colors">
          {item.icon}
        </div>
    ...
    <section ref={ref} className="px-4 md:px-12 py-10 md:py-14 bg-[#141715] text-white border-y border-neutral-800">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8 pb-4 border-b border-neutral-800">
          <div>
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded bg-[#0B3A66]/20 border border-[#0B3A66]/40 text-[#85B7EB] font-['Space_Mono'] text-[10px] font-bold uppercase tracking-wider mb-2">
              <TrendingUp size={12} />
